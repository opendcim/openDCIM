...
if (isset($_POST["NewSPCert"])) {
	...
	$req_key = new OpenSSLAsymmetricKey;
  ...
  <![endif]-->
	...
	<script type="text/javascript">
		...
		function rebuildcache() {
			$('<div>').append($('<iframe>').attr('src', 'build_image_cache.php').css({
				'max-width': '600px',
				'max-height': '400px',
				'min-height': '300px',
				'align': 'middle'
			})).attr('title', 'Rebuild device image cache').dialog({
				width: 'auto',
				modal: true,
				open: function(e) {
					thismodal = this;
				...
				$('#PDFLogoFile').click(function() {
						...
						$.get('', {
							fl: this.value
								...
								$("#imageselection").html(data);
								var upload = $('<input>').prop({
									type: 'file',
									name: 'dev_file_upload',
									id: 'dev_file_upload'
								}).data('dir', 'images');
								...
								}).data('input', input);; bindimageselection(); $("#imageselection").next('div').prepend(upload); uploadifive();
						});
				});
			...
			$('#drawingpath, #picturepath, #reportspath').click(function() {
					...
					$.get('', {
						dl: this.value
							...
							}).data('input', input); binddirectoryselection();
					});
		...
		function removemedia(row) {
			$.post('', {
				mtused: row.find('div:nth-child(2) input').attr('data')
			}).done(function(data) {
					if (data.trim() == 0) {
						...
						var defaultbutton = {
							"<?php echo __("Clear all"); ?>": function() {
								$.post('', {
									mtid: row.find('div:nth-child(2) input').attr('data'),
									mt: '',
									mtcc: '',
									clear: ''
								}).done(function(data) {
					...
					choices.find('option').each(function() {
						if ($(this).val() == row.find('div:nth-child(2) input').attr('data')) {
							$(this).remove();
						}
					});
		...
		function removecolor(rowobject, lookup) {
			if (!lookup) {
				...
				$.get('api/v1/colorcode/' + rowobject.find('div:nth-child(2) input').attr('data') + '/timesused').done(function(data) {
						if (data.colorcode == 0) {
							$.ajax('api/v1/colorcode/' + rowobject.find('div:nth-child(2) input').attr('data'), {
								type: 'delete'
							}).done(function(data) {
								if (!data.error) {
									updatechoices();
					...
					var choices = $('div#mediatypes.table div:last-child div select').clone(); choices.find('option').each(function() {
						if ($(this).val() == rowobject.find('div:nth-child(2) input').attr('data')) {
							$(this).remove();
						}
					}); choices.change(function() {
						if ($(this).val() == '') { // clear all
			...
			row.find('div > input').each(function() {
				...
				$(this).change(function() {
					if (cc.val().trim() != '') {
						...
						if (cc.attr('data')) {
							$.post('api/v1/colorcode/' + cc.attr('data'), {
								ColorID: cc.attr('data'),
								Name: cc.val(),
								DefaultNote: ccdn.val()
							}).done(function(data) {
								if (data.error) {
		...
		function binddcarow(row) {
			...
			if (row.Label.val().trim() == '' && row.addrem.prop('id') != 'newline') {
				...
				if (((row.addrem.prop('id') == 'newline' && row.Label.val() != '') || row.addrem.prop('id') != 'newline') && $(".main form").validationEngine('validate')) {
					$.post('', {
						dcal: dcal.val(),
						dcaid: dcal.attr('data'),
						dcat: dcat.val(),
						dcar: dcar.prop('checked'),
						dcaa: dcaa.prop('checked'),
						dcav: dcavtosend
					}).done(function(data) {
						if (data.trim() == 'f') { //fail
							revertdefault(row, true);
				...
				$.post('', {
					...
					var defaultbutton = {
						"<?php echo __("Delete from All Devices/Templates"); ?>": function() {
							$.post('', {
								dcaid: row.find('div:nth-child(2) input').attr('data'),
								dcal: '',
								dcar: '',
								dcaa: '',
								dcav: '',
		...
		function createstatusrow(statusobject) {
			...
			newrow.append($('<div>').addClass('addrem').append($('<img>').attr({
				'src': 'images/del.gif',
				'height': 20,
				'width': 20
			})));
			...
			newrow.append($('<div>').append($('<div>').addClass('cp').append($('<input>').attr({
				'type': 'text',
				'name': 'StatusColor'
			}).val(statusobject.ColorCode).addClass('color-picker'))));
		...
		function updatestatus(e) {
			...
			if (row.Label.val() != '' && $(".main form").validationEngine('validate')) {
				$.ajax({
					...
					data: {
						'StatusID': row.ID,
						'Status': row.Label.val(),
						'ColorCode': row.Color.val()
					},
					success: function(data) {
						if (!data.error) {
			...
			$('#dev_file_upload').uploadifive({
				...
				'onUploadComplete': function(file, data) {
					...
					if (data.status == '1') {
						...
						var close = $('<a>').addClass('close').text('X').click(function() {
							$(this).parent('div').remove();
						});
						var span = $('<span>');
						var error = $('<div>').addClass('border').css({
							'margin-top': '2px',
							'padding': '3px'
						}).text(data.msg);
						toast.append(close);
   ...
   <input type="hidden" name="Version" value="', $config->ParameterArray["Version"], '">
	...
	<div id="configtabs">
		...
		<div id="general">
			...
			<h3>', __("Image Lengths"), '</h3>
			<div class="table" id="rackusage">
				<div>
					<div><label for="ImgLenContainerMark">', __("Container Marks"), '</label></div>
					<div><input type="text" defaultvalue="', $config->defaults["ImgLenContainerMark"], '" name="ImgLenContainerMark" value="', $config->ParameterArray["ImgLenContainerMark"], '"></div>
				</div>
			</div> <!-- end table -->
		</div>
		<div id="workflow">
