...
class DataCenter
{
	...
	function GetChildren()
	{
		...
		foreach ($cab->ListCabinetsByDC() as $child) {
			if ($child->ZoneID > 0 || $child->CabRowID > 0 || $child->DataCenterID != $this->DataCenterID) { } else {
				$children[] = $child;
			}
		}
	...
	function GetOverview()
	{
		...
		if (strlen($this->DrawingFileName)) {
			...
			$tempSQL = "select max(TimeStamp) as ReadingTime from fac_SensorReadings where DeviceID in (select DeviceID from fac_Device where DeviceType='Sensor' and Cabinet in (select CabinetID from fac_Cabinet where DataCenterID=" . $this->DataCenterID . "))";
