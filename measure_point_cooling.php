<?php
	...
	$subheader=__("Data Center Cooling Measure Point Detail");
	...
	if(isset($_REQUEST['action']) && (($_REQUEST['action']=='Create')||($_REQUEST['action']=='Update'))){
		if($_REQUEST['connectiontype'] == "SNMP") {
			$mp = new SNMPCoolingMeasurePoint();
			...
			$mp = new ModbusCoolingMeasurePoint();
	...
	$mpList = new CoolingMeasurePoint();
...
echo '							</select></div>
						...
						<div id="modbus_fanspeedregister">
...
function OnConnectionTypeChange() {
	var snmp = ['com', 'version', 'fanspeedoid', 'coolingoid'];
	var modbus = ['unit', 'nbwords', 'fanspeedregister', 'coolingregister'];
