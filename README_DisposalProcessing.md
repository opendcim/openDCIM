...
4. **Actions**: ignore/create minimal entries, move to storage, confirm disposal, etc.  
5. **Finalize**: choose disposition method/date, run `Device::Dispose()` for confirmed devices and log each action.  
6. **Log & Rollback**: download CSV log; rollback restores cabinet/position and removes fac_DispositionMembership updates.  
7. **Reset**: reloading the page resets the wizard for a new file.
...
4. **Actions** : ignorer/créer des entrées minimales, déplacer en Storage Room, confirmer la méthode de sortie, etc.  
5. **Finaliser** : choisir méthode/date, lancer `Device::Dispose()` pour chaque équipement confirmé avec journalisation.  
6. **Journal & rollback** : export CSV, bouton d’annulation restaurent armoires/positions et nettoient fac_DispositionMembership.  
7. **Réinitialisation** : actualiser la page (GET) redémarre l’assistant pour un nouveau fichier.
...
- Télécharger le journal après chaque session pour tracer les opérations.
