	...
	if (isset($_POST['type']) && $_POST['type'] == 'cabinetid') {
		...
		if ($cab->Rights != "None") {
			...
			$sql = "SELECT MAX(Temperature) AS Temperature, MAX(Humidity) AS Humidity, 
				MAX(TimeStamp) AS LastRead FROM fac_SensorReadings WHERE DeviceID IN (SELECT 
			...
			if (!isset($cab->CabinetHeight) || $cab->CabinetHeight == 0) {
				$SpacePercent = 100;
			} else {
				$SpacePercent = locale_number($used / $cab->CabinetHeight * 100, 0);
			}
			// check to make sure there is a weight limit set to keep errors out of logs
			if (!isset($cab->MaxWeight) || $cab->MaxWeight == 0) {
				$WeightPercent = 0;
			} else {
				$WeightPercent = locale_number($totalWeight / $cab->MaxWeight * 100, 0);
			}
			// check to make sure there is a kilowatt limit set to keep errors out of logs
			if (!isset($cab->MaxKW) || $cab->MaxKW == 0) {
				$PowerPercent = 0;
			} else {
				$PowerPercent = locale_number(($totalWatts / 1000) / $cab->MaxKW * 100, 0);
			}
			if (!isset($cab->MaxKW) || $cab->MaxKW == 0) {
				$RealPowerPercent = 0;
			} else {
				$RealPowerPercent = locale_number(($curRealPower / 1000) / $cab->MaxKW * 100, 0);
			}
			...
			if ($curTemp == 0) {
				$tcolor = $us;
			} elseif ($curTemp > $TempRed) {
				$tcolor = $rs;
			} elseif ($curTemp > $TempYellow) {
				$tcolor = $ys;
			} else {
				$tcolor = $gs;
			}
			if ($curHum == 0) {
				$hcolor = $us;
			} elseif ($curHum > $HumMax || $curHum < $HumMin) {
				$hcolor = $rs;
			} elseif ($curHum > $HumMedMax || $curHum < $HumMedMin) {
				$hcolor = $ys;
			} else {
				$hcolor = $gs;
			}
		...
		if ($tooltip == "") {
			$tooltip = __("Tooltips are enabled with no options selected.");
		}
	}
}
